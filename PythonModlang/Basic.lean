...
| fOp (s : S) : (args : ∀ i : Fin (σ.arity s), FreeM σ (σ.argTy i)) →
                FreeM σ (σ.outTy s)
...
structure Env {S : Type} (σ : Signature S) (m : Type → Type) where
  evalS (s : S) : (∀ i : Fin (σ.arity s), m (σ.argTy i)) → m (σ.outTy s)
...
| fBind x f => do
  ...
  evalFreeM env (f a)
| fOp s f => env.evalS s (fun i => evalFreeM env (f i))


def iNil (n : Fin 0) : α := by cases n; omega

def iCons (a : α) (v : Fin n → α) : Fin (n+1) → α := Fin.cases a (fun j => v j)
...
partial def addToPython (e : Lean.Expr) : MetaM PyVal :=
  if e.getAppFn.constName! == ``HAdd.hAdd then
    do
      dbg_trace s!"{e}"
      dbg_trace s!"{e.getArg! 4}"
      let arg₁ ← exprToPython <| e.getArg! 4
      dbg_trace s!"{e.getArg! 5}"
      let arg₂ ← exprToPython <| e.getArg! 5
      return paren <| arg₁ ++ " + " ++ arg₂
  else
  failure
